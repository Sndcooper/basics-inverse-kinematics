...
xy_max = HIP_RADIUS + max(abs(r_min), abs(r_max))
...
def update(_):
    ...
    maxr = max(abs(x), abs(y), abs(z), HIP_RADIUS + l1 + l2 + l3) + 0.5
    ...
    for idx, hip_pos in enumerate(HIP_POSITIONS):
        ...
        ca = cos(-leg_angle)
        sa = sin(-leg_angle)
        leg_x = ca * local_x - sa * local_y
        leg_y = sa * local_x + ca * local_y

        leg_off_plane = abs(leg_y)
        if leg_off_plane > LEG_Y_TOL:
            result = None
        else:
            result = solve_ik_3d(leg_x, 0.0, local_z, l1, l2, l3, foot_angle)

        if result is None:
            leg_points = offset_points(default_points, hip_pos)
        else:
            base_yaw, angle1, angle2, angle3, phi, psi, r = result
            base_yaw_world = leg_angle
            if not (
                ...
                reachable_count += 1
                fk_points = forward_kinematics(base_yaw_world, angle1, angle2, angle3, l1, l2, l3)
                leg_points = offset_points(fk_points, hip_pos)
            ...
            if idx == 0:
                ...
                first_leg_text = "\n".join(
                    [
                        f"base_yaw = {base_yaw_world:.3f} rad ({deg(base_yaw_world):.1f} deg)",
        ...
        xs = [p[0] for p in leg_points]
        ys = [p[1] for p in leg_points]
        zs = [p[2] for p in leg_points]
